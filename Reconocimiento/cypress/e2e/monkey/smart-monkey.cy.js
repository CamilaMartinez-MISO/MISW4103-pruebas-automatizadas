        ...
        if($inputs.length > 0){
            ...
            if(!Cypress.dom.isHidden(inp)) {
                ...
                if(inp.getAttribute("type") == "email"){
                    ...
                    let type = faker.date.anytime();
                    cy.wrap(inp).type(faker.date.anytime());
                    info = `Input ${inp.id} was filled with ${type}`
                }
                else if(inp.getAttribute("type") == "tel"){
                    let type = faker.phone.number();
...
describe( `${appName} under smarter monkeys`, function() {
    ...
    it(`visits ${appName} and survives smarter monkeys`, function() {
        ...
        if(pcg === 100){
            ...
            cy.log(`Page max X: ${curPageMaxX}, Page max Y: ${curPageMaxY}`)
            cy.wait(500)
            cy.get('input[name="identification"]').type('h.franco@uniandes.edu.co')
            cy.wait(500)
            cy.get('input[name="password"]').type('miso20244103')
            cy.wait(500)
